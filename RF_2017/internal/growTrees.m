...
%       2   3                 trees.dim       - feature dimension of split function  // vertical or horizontal  
...
for T = 1:param.num
    ...
    idx = randsample(N,N,1); % A new training set for each tree is generated by random sampling from dataset WITH replacement.
    ...
    % Split Nodes
    for n = 1:2^(param.depth-1)-1  %Split Nodes
        [tree(T).node(n),tree(T).node(n*2),tree(T).node(n*2+1)] = splitNode(data,tree(T).node(n),param);
    end
    
    % Leaf Nodes
    cnt = 1;
    for n = 1:2^param.depth-1   %// total number of nodes including split and leaf
        if ~isempty(tree(T).node(n).idx) % //if not a leaf
